...
bac <- readxl::read_xlsx("HC-504-Distribution-moyenne-bac-2018-2020-filière-série.xlsx", 
  ...
  mutate(Grade = cut(Moyenne, right = FALSE,
                       breaks=c(0,10,12,14,16,21),
                       labels = c("Fail","Standard pass", "Honour","High honour","Highest honour")))
...
bac %>%
  ggplot(aes(x=Moyenne,y=Tout.Effectif,color=Grade,fill=Grade)) +
  geom_col() +
  xlab("Average overall grade, over 20") + 
...
```{r bac.distrib.s}
bac %>%
  ggplot(aes(x=Moyenne,y=BacGen_S.Effectif,color=Grade,fill=Grade)) +
  geom_col() 
```
...
```{r bac.distrib.1}
bac %>%
  group_by(Moyenne = floor(Moyenne), Grade) %>%
...
bac %>%
  group_by(Grade) %>%
  summarise(Tout.Effectif = sum(Tout.Effectif)) %>%
  ggplot(aes(x=Grade,y=Tout.Effectif,color=Grade,fill=Grade)) +
  geom_col() +
  xlab("Grade") + 
  ylab("Number of candidates") 
...
ps20 %>%
  ...
  mutate(Grade = factor(Grade,levels=c("fail","sans.mention.au.bac","avec.mention.Assez.Bien.au.bac","avec.mention.Bien.au.bac","avec.mention.Très.Bien.au.bac"), labels=c("Fail","Standard\npass","Honour","High\nhonour","Highest\nHonour"))) %>%
  ...
  ggplot(aes(x=Grade,y=Admis,fill=Grade)) +
  geom_col() +
  facet_wrap(.~label) +
...
menratio <- bac %>%
  ...
  filter(Grade != "Non admis") %>%
  mutate(
    Ratio = Tout.Effectif / sum(Tout.Effectif),
    Ratio.max = Max / Tout.Effectif) %>%
  mutate(Grade = factor(Grade, labels=c("Fail","Standard\npass","Honour","High\nhonour","Highest\nHonour"))) 
...
ps20 %>%
  ...
  mutate(Grade = factor(Grade,levels=c("sans.mention.au.bac","avec.mention.Assez.Bien.au.bac","avec.mention.Bien.au.bac","avec.mention.Très.Bien.au.bac"), labels=c("Standard\npass","Honour","High\nhonour","Highest\nHonour"))) %>%
  left_join(menratio) %>%
  ...
  ggplot(aes(x=Grade,y=Nombre,fill=Grade, alpha=Type)) +
