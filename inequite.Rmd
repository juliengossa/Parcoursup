...
bac <- readxl::read_xlsx("HC-504-Distribution-moyenne-bac-2018-2020-filière-série.xlsx", 
  ...
  mutate(Mention = cut(Moyenne, right = FALSE,
                       breaks=c(0,10,12,14,16,21),
                       labels = c("Non admis","Sans mention","Assez bien","Bien","Très bien")))
...
```{r bac.distrib}
bac %>%
  ggplot(aes(x=Moyenne,y=Tout.Effectif,color=Mention,fill=Mention)) +
  geom_col()
```
...
```{r bac.distrib.s}
bac %>%
  ggplot(aes(x=Moyenne,y=BacGen_S.Effectif,color=Mention,fill=Mention)) +
  geom_col() 
```
...
```{r bac.distrib.1}
bac %>%
  group_by(Moyenne = floor(Moyenne), Mention) %>%
...
bac %>%
  group_by(Mention) %>%
  summarise(Tout.Effectif = sum(Tout.Effectif)) %>%
  ggplot(aes(x=Mention,y=Tout.Effectif,color=Mention,fill=Mention)) +
  geom_col()
```
...
ps20 %>%
  ...
  mutate(Mention = factor(Mention,levels=c("sans.mention.au.bac","avec.mention.Assez.Bien.au.bac","avec.mention.Bien.au.bac","avec.mention.Très.Bien.au.bac"), labels=c("Sans","AB","B","TB"))) %>%
  mutate(label = paste0("(",id,") ",Effectif.total.des.candidats.en.phase.principale," candidats / ",Capacité.de.l.établissement.par.formation," places")) %>%
  ggplot(aes(x=Mention,y=Admis,fill=Mention)) +
  geom_col() +
  facet_wrap(.~label)
```
...
```{r ps.distrib.max.mention.cand}
menratio <- bac %>%
  group_by(Mention) %>%
  summarise(
    Tout.Effectif = sum(BacGen_S.Effectif),
    Max = max(BacGen_S.Effectif)) %>%
  filter(Mention != "Non admis") %>%
  mutate(
    Ratio = Tout.Effectif / sum(Tout.Effectif),
    Ratio.max = Max / Tout.Effectif) %>%
  mutate(Mention = factor(Mention,labels=c("Aucune","AB","B","TB")))
...
ps20 %>%
  ...
  mutate(Mention = factor(Mention,levels=c("sans.mention.au.bac","avec.mention.Assez.Bien.au.bac","avec.mention.Bien.au.bac","avec.mention.Très.Bien.au.bac"), labels=c("Aucune","AB","B","TB"))) %>%
  left_join(menratio) %>%
  ...
  mutate(label = paste0("(",id,") ",Effectif.total.des.candidats.en.phase.principale," candidats / ",Capacité.de.l.établissement.par.formation," places")) %>%
  ggplot(aes(x=Mention,y=Nombre,fill=Mention, alpha=Type)) +
